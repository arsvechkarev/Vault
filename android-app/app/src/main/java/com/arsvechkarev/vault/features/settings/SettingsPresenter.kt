...
import com.arsvechkarev.vault.features.settings.SettingsScreenActions.*
import com.arsvechkarev.vault.features.settings.SettingsScreenSingleEvents.ShowBiometricsAddedSuccessfully
import com.arsvechkarev.vault.features.settings.SettingsScreenUserActions.*
    ...
    private fun subscribeToBiometricsEvents() {
        biometricsPrompt.biometricsEvents().collectInPresenterScope { event ->
            when (event) {
                is Success -> {
                    onIoThread {
                        biometricsStorage.encryptAndSaveData(
                            masterPassword,
                            event.result
                        )
                    }
