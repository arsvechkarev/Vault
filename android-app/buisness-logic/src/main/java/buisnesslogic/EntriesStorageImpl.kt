...
class EntriesStorageImpl(
  ...
  override suspend fun getEntries(masterPassword: Password): Entries {
    //    val ciphertext = checkNotNull(databaseSaver.read()) { "Encrypted data was empty" }
    //     Text in file should not be empty, because it should have been saved earlier
    //    require(ciphertext.isNotEmpty())
    val json = cryptography.decryptData(masterPassword, "".toByteArray())
    //    if (json == "") return Entries(emptyList(), emptyList(), emptyList())
    return gson.fromJson(json, Entries::class.java)
  }
  
  override suspend fun saveEntries(masterPassword: Password, entries: Entries) {
    saveEntriesToFile(masterPassword, entries)
  }
  
  override suspend fun saveEntry(masterPassword: Password, entry: Entry): Entries {
    val entries = getEntries(masterPassword)
    val newEntries = when (entry) {
      is PasswordEntry -> entries.copy(passwords = entries.passwords + entry)
  ...
  override suspend fun updateEntry(masterPassword: Password, entry: Entry): Entries {
    val entries = getEntries(masterPassword)
    val newEntries = when (entry) {
      is PasswordEntry -> entries.copy(
  ...
  override suspend fun deleteEntry(masterPassword: Password, entry: Entry): Entries {
    val entries = getEntries(masterPassword)
    val newEntries = when (entry) {
      is PasswordEntry -> entries.copy(
