...
class MainController {
  ...
  fun onCopyWebsiteNameClicked() {
    copyText(imageCopyWebsiteName, requireNotNull(currentPassword).websiteName)
  }
  
  fun onCopyLoginClicked() {
    copyText(imageCopyLogin, requireNotNull(currentPassword).login)
  }
  
  fun onCopyPasswordClicked() {
    copyText(imageCopyPassword, requireNotNull(currentPassword).password)
  }
  ...
  fun onCopyNotesClicked() {
    copyText(imageCopyNotes, requireNotNull(currentPassword).notes)
  }
  
  fun onCopyTitleClicked() {
    copyText(imageCopyTitle, requireNotNull(currentPlainText).title)
  }
  
  fun onCopyTextClicked() {
    copyText(imageCopyText, requireNotNull(currentPlainText).text)
  }
  ...
  private fun addNotification() {
    notification = Pane().apply {
      ...
      children.add(ImageView().apply {
        image = Image(VaultDesktopApplication::class.java.getResourceAsStream("icons/check.png"))
        layoutX = 8.0
        layoutY = 5.0
        fitHeight = 30.0
        fitWidth = 30.0
  ...
  private fun copyText(element: Node, text: String) {
    Clipboard.getSystemClipboard().setContent(ClipboardContent().apply { putString(text) })
    TranslateTransition().apply {
      duration = Duration.millis(600.0)
      node = notification
      byY = -100.0
      element.isMouseTransparent = true
      play()
      setOnFinished {
        PauseTransition().apply {
          ...
          setOnFinished {
            TranslateTransition().apply {
              ...
              play()
              setOnFinished {
                element.isMouseTransparent = false
              }
            }
          }
        }
